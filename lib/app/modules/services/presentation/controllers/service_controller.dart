...
class ServiceController = _ServiceControllerBase
    with _$ServiceController;
...
abstract class _ServiceControllerBase with Store {
  ...
  String convertTime(double time){
    var fullTime = time * 100;
    if(fullTime < 100){
      return '$fullTime min';
    }else{
      var hour = fullTime.toString().substring(0, fullTime.toString().length -2);
